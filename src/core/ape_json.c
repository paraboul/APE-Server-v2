...
void set_json(const char *name, const char *value, struct json **jprev)
{
	...
	new_json->name.buf = malloc(sizeof(char) * (new_json->name.len + 1));
	memcpy(new_json->name.buf, name, new_json->name.len + 1);
	
	new_json->value.len = (value != NULL ? strlen(value) : 0);
	
	if (new_json->value.len) {
		new_json->value.buf = malloc(sizeof(char) * (new_json->value.len + 1));
...
{
	struct json *new_json = malloc(sizeof(struct json));
	...
	while (jchilds != NULL) {
		struct json_childs *new_child = malloc(sizeof(struct json_childs));
...
void json_attach(struct json *json_father, struct json *json_child, unsigned int type)
{
	...
	nchild = malloc(sizeof(struct json_childs));
...
static int escape_json_string(const char *in, char *out, int len)
{
	int i, e;
...
{
	if (string == NULL) {
		string = malloc(sizeof(struct jsontring));
		...
		string->jstring = malloc(sizeof(char) * (string->jsize + 1));
		string->len = 0;
	}
...
json_item *json_item_copy(json_item *cx, json_item *father)
{
	...
	while (cx != NULL) {	
		...
		if (cx->key.val != NULL) {
			...
			new_item->key.val = malloc(sizeof(char) * (cx->key.len + 1));
			memcpy(new_item->key.val, cx->key.val, cx->key.len + 1);
		}
		
		if (cx->jval.vu.str.value != NULL) {
			new_item->jval.vu.str.length = cx->jval.vu.str.length;
			new_item->jval.vu.str.value = malloc(sizeof(char) * (cx->jval.vu.str.length + 1));
...
void json_set_property_objN(json_item *obj, const char *key, int keylen, json_item *value)
{
	...
	if (key != NULL) {
		new_item->key.val = malloc(sizeof(char) * (keylen + 1));
...
void json_set_property_strN(json_item *obj, const char *key, int keylen, const char *value, int valuelen)
{
	...
	new_item->jval.vu.str.value = malloc(sizeof(char) * (valuelen + 1));
...
int json_callback(void *ctx, int type, const JSON_value* value)
{
	...
	switch(type) {
		...
		case JSON_T_KEY:
			...
			cx->current_cx->key.val = malloc(sizeof(char) * (value->vu.str.length+1));
		...
		case JSON_T_STRING:
			...
			switch(type) {
				...
				case JSON_T_TRUE:
					...
					break;
				case JSON_T_STRING:
					cx->current_cx->jval.vu.str.value = malloc(sizeof(char) * (value->vu.str.length+1));
